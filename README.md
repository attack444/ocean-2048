...
> оценивается через удержание. Подробный план развития: [`docs/ROADMAP.md`](docs/ROADMAP.md)
...
### Глубина ядра (Фаза 2.5) — в работе (2/5)
- **Прилив 🌊** (готово): каждые N ходов вода поднимается и смывает нижний ряд —
  плитки «уносит течением», за них возвращается 50% очков. Индикатор 🌊 над доской
  показывает высоту воды и число ходов до прилива; на глубоких уровнях прилив
  сильнее (больше рядов, меньше интервал). Счётчик/смытые плитки сохраняются,
  откатываются через «Отменить».
- **Ходы как ресурс 🧮** (готово): «бесполезный» ход (без единого слияния)
  наказывается — прибавляет шаг прилива (прилив приходит быстрее) и накапливает
  «напряжение»: N бесполезных ходов подряд вызывают «водоворот» 🌪️ — смыв нижних
  рядов **без** возврата очков. Индикатор 🌪️ показывает, сколько ходов осталось до
  штрафа. Каждый свайп обязан что-то давать. Счётчики сохраняются и откатываются
  через «Отменить».
- Следующие механики (в плане): позиционные цели 🪸, акула-охотник 🦈,
  плитки-способности ⚡.
...
```bash
npm install
npm run test           # 238 юнит-тестов (node:test)
...
| 2.5 | «Глубина ядра» (главный приоритет): **Прилив 🌊 — готово, ходы как ресурс 🧮 — готово**, позиционные цели, акула-охотник, плитки-способности | 🟡 в работе (2/5) |
| 3 | Публикация в каталог VK (модерация) | ⬜ план |
| 4 | Серверные механики VK: лента, миссии, уведомления (после каталога) | ⬜ план |
| 5 | «UI/UX полировка»: онбординг (6 шагов), доступность (⚡ быстрые анимации, 🔍 крупный текст), плавные модалки | 🟡 в работе (4/6) |
| 6 | «Смена стиля от прогресса» + рост retention: сюжетные миссии, модификаторы, события, режимы | ⬜ план |
| 7 | «Живой океан» (масштаб): ежедневная головоломка, сезоны, океанариум, боссы, гильдии | ⬜ план |
| 8 | Своя 24/7-экосистема: свой хостинг, бэкенд, аналитика | ⬜ план |
