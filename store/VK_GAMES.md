# VK Игры (VK Mini Apps) — публикация «Океан 2048»

Гайд по публикации браузерной версии игры на платформе **VK Игры** (VK Mini Apps,
`vk.com` / dev.vk.com). В отличие от Яндекс Игр, VK публикует игру **не архивом,
а по HTTPS-URL** (раздел «Размещение» в панели управления приложением): игра
грузятся в **iframe** VK, поэтому все файлы должны отдаваться по HTTPS и сервер
не должен слать заголовок `X-Frame-Options: DENY/SAMEORIGIN`.

Официальная документация:
- Обзор VK Mini Apps: https://dev.vk.com/ru/mini-apps/overview
- Начало работы: https://dev.vk.com/ru/mini-apps/getting-started
- Запуск приложения (iframe, параметры запуска): https://dev.vk.com/ru/mini-apps/launch-game
- Модерация каталога: https://dev.vk.com/ru/mini-apps/catalog/moderation

Обновлён: 2026-09-08.

> 🌐 **Одноклассники (OK):** ОК — часть единой платформы VK Games (тот же
> VK Bridge, тот же кабинет dev.vk.com, то же приложение 54731343). Отдельного
> кода не нужно — только привязка «OK ID» в настройках и модерация каталога OK.
> Полный гайд по OK (шаги, чек-лист модерации, CSP): [`OK_GAMES.md`](OK_GAMES.md).

## ⚠️ Ответ модерации VK (2026-09-08) — что исправить перед повторной подачей

Модерация каталога VK вернула замечания. Ниже — разбор каждого пункта и
пошаговые действия (все три — в кабинете dev.vk.com; код игры уже готов).

### №1. Механики больше подходят категории «Игры» — сменить формат и подать заново

Приложение создано/настроено как **«Приложение»**, а не **«Игра»**. VK требует,
чтобы мини-апп с игровыми механиками был типа **«Игра»** (тогда он попадает в
каталог VK Игры и проходит игровую модерацию).

**Действия в кабинете dev.vk.com (приложение 54731343):**
1. Открыть приложение → **«Настройки»**.
2. В поле **«Тип приложения»** выбрать **«Игра»** (Game), а не «Приложение»/«Mini App».
3. Сохранить. При необходимости — заново заполнить карточку каталога
   (категория **«Головоломки»**, возрастной рейтинг 3+, описание — см. ниже).
4. Подать на модерацию заново.

> Код игры от типа не зависит — меняется только настройка в кабинете.
> Если в кабинете нет переключателя типа — создать **новый** мини-апп типа
> «Игра» с тем же App ID-механизмом и перенести URL из раздела «Размещение».

### №2. Интерфейс должен быть удобен без скролла экрана (ПК и телефон)

В `css/styles.css` уже реализована компактная адаптивная вёрстка под весь
экран: `html, body { overflow: hidden }`, контейнер на `100dvh`, доска
масштабируется под свободную высоту через JS-переменную `--board-max`,
container-query и медиа-запросы для `body.is-native` / `body.is-ios`.
Проверка — в браузере (локально) и в iframe VK на ПК и телефоне: главный экран
(шапка + статистика + доска + управление) должен помещаться без прокрутки.
Если на конкретном устройстве что-то переполняется — уменьшить отступы/шрифты
в соответствующих медиа-запросах (см. `css/styles.css`, блоки `@media` и
`body.is-native`).

### №3. Внедрить тестовые платежи за голоса (VKWebAppShowOrderBox) для модерации

Код платежей **полностью готов и протестирован**:
- [`js/platform-sdk.js`](../js/platform-sdk.js): `getDonateCatalog()` →
  `VKWebAppGetOrderItems`, `buyDonate(itemId)` → `VKWebAppShowOrderBox`
  (юнит-тесты в `platform-sdk.test.js` — pass).
- [`js/chest.js`](../js/chest.js): наборы `DONATE_PACKS` (donate_small/medium/large/mega).
- [`js/main.js`](../js/main.js): магазин жемчужин → `handleDonate()` → `buyDonate()`;
  в VK цена показывается в голосах, есть пометка «тестовый режим до публикации».

Осталось **активировать товары и монетизацию в кабинете** и проверить в песочнице:

1. **Создать товары** в кабинете dev.vk.com → приложение 54731343 →
   **«Платежи»** → «Товары» → «Создать товар» (4 шт., id/название должны
   совпадать с `DONATE_PACKS` в [`js/chest.js`](../js/chest.js)):
   | id | Название | Цена (голоса) |
   |----|----------|---------------|
   | `donate_small` | Мешок жемчуга | 10 |
   | `donate_medium` | Сундук с жемчугом | 25 |
   | `donate_large` | Королевская шкатулка | 60 |
   | `donate_mega` | Сокровищница глубин | 130 |
2. **Включить монетизацию** приложения: «Платежи» → включить платежи за голоса
   (и рекламу interstitial/rewarded, если нужна).
3. **Проверить в песочнице** (тестовый режим VK): открыть игру → магазин →
   нажать «Купить» у набора → должно открыться системное окно
   `VKWebAppShowOrderBox` → подтвердить → зачисляются жемчужины.
   В тестовом режиме VK даёт тестовые голоса без реального списания.
4. Убедиться, что в консоли нет ошибок `VKWebAppGetOrderItems` /
   `VKWebAppShowOrderBox`.
5. Подать на модерацию заново, указав в «Описании основных сценариев» шаг
   «покупка жемчужин за голоса через системное окно VK».

## Что уже реализовано (SDK-адаптер `js/platform-sdk.js`)

| Возможность | API VK | Статус |
|-------------|--------|--------|
| Инициализация | `VKWebAppInit` (обязателен для VK) | ✅ |
| Автоопределение хоста | `detectHost()`: `vkBridge` / `VKWebApp` / `vk_app_id` / `vk_platform` / `vk_user_id` → `'vk'` | ✅ |
| Подключение VK Bridge | динамически `https://unpkg.com/@vkontakte/vk-bridge/dist/browser.min.js` (`ensureBridge()`) | ✅ |
| Облачные сохранения | `VKWebAppStorageSet` / `VKWebAppStorageGet` (ключ `ocean2048_save`) | ✅ |
| Лидерборд (запись) | ~~`VKWebAppSaveToLeaderBoard`~~ — **удалён из vk-bridge**; актуально: `secure.addAppEvent` (сервер, после каталога) | ❌ см. Фазу 0.5 |
| Лидерборд (показ) | `VKWebAppShowLeaderBoardBox` (`user_result`) — системная таблица | ❌ добавить (Фаза 0.5) |
| Реклама interstitial | `VKWebAppShowNativeAds` (`ad_format: 'interstitial'`) | ✅ |
| Реклама rewarded | `VKWebAppShowNativeAds` (`ad_format: 'reward'`) | ✅ |
| Покупки за голоса (IAP) | `VKWebAppGetOrderItems` (каталог) + `VKWebAppShowOrderBox` (`type:'item'`) | ✅ код готов; нужна активация товаров в кабинете «Платежи» |
| Поделиться | `VKWebAppShare` | ✅ |

Все методы безопасны: вне VK (web/standalone) возвращают `false`/пустые данные,
игра продолжает работать без VK Bridge.

> ⚠️ **КРИТИЧНО:** методы `VKWebAppSaveToLeaderBoard` / `VKWebAppGetLeaderBoard`
> **удалены из vk-bridge** (проверено в 3.0.2 и 2.2.2). Актуальный механизм VK —
> серверный `secure.addAppEvent` (запись, после каталога) + `VKWebAppShowLeaderBoardBox`
> (показ системной таблицы с друзьями). Подробности и план замены — в
> [`store/DEV_PLAN.md`](DEV_PLAN.md) §2.1 и Фазе 0.5.

> ⚠️ На VK облачные сохранения работают **без авторизации** — это платформенное
> хранилище VK (в отличие от Яндекс Игр). Ключ облака `ocean2048_save` отличается
> от ключа Яндекса `ocean2048` — конфликтов между платформами нет.

## Ключевое отличие: публикация по HTTPS-URL, а не архивом

| | Яндекс Игры | VK Игры |
|---|---|---|
| Формат загрузки | ZIP-архив | **HTTPS-URL** (раздел «Размещение») |
| Хостинг | на серверах Яндекса | **на своём сервере** (или бесплатный `vk-miniapps-deploy`) |
| Игра грузится | в iframe Яндекса | в **iframe VK** |
| SDK | `/sdk.js` (Яндекс) | VK Bridge (`@vkontakte/vk-bridge`) |

У нас игра уже развёрнута на HTTPS:
- Плейграунд: https://5mb2.ru/games/game/ocean-2048
- Статика (прямой URL для VK): **https://5mb2.ru/static/games/ocean-2048/index.html**
- HTTPS настроен (Certbot), политика конфиденциальности: https://5mb2.ru/static/games/ocean-2048/privacy-policy.html

## Требования VK к хостингу и странице

1. **HTTPS обязателен** — VK не грузит мини-апп по http.
2. **Заголовок `X-Frame-Options`**: сервер **не должен** слать
   `X-Frame-Options: DENY` или `SAMEORIGIN` (и `Content-Security-Policy: frame-ancestors`,
   запрещающую площадку), иначе iframe VK/OK не отобразит игру. Нужно разрешить
   `frame-ancestors https://vk.com https://*.vk.com https://*.vk.me
   https://ok.ru https://*.ok.ru https://m.ok.ru` (окружение OK добавлено 2026-08-25).
3. **`VKWebAppInit` обязателен** — вызывается в `init()` платформенного адаптера.
4. **Параметры запуска**: VK передаёт в iframe query-параметры
   (`vk_user_id`, `vk_app_id`, `vk_are_ads_enabled`, `vk_is_app_user`, `vk_language`,
   `sign` и др.) — на них завязано автоопределение хоста в `detectHost()`.
5. Игра должна работать **без ошибок в консоли** (требование модерации каталога).

Проверка X-Frame-Options для нашего сервера (5mb2.ru):
```bash
curl -sI https://5mb2.ru/static/games/ocean-2048/index.html | findstr /i "x-frame-options content-security-policy"
# Если обе строки отсутствуют — iframe VK откроется. Если есть — добавить на сервер
# заголовок, разрешающий frame-ancestors vk.com (см. выше).
```

> ✅ **Исправлено (2026-08-21):** сервер слал `X-Frame-Options: DENY` и CSP
> `frame-src` без `frame-ancestors` для `/static/games/*` — это гарантировало
> «белый экран» в iframe VK. Правка в `services/web/src/middleware/security.js`
> на сервере: для путей `/static/games/*` заголовок `X-Frame-Options` НЕ
> выставляется, а в CSP вместо `frame-src` добавляется
> `frame-ancestors 'self' https://vk.com https://*.vk.com https://*.vk.me`.
> Для остальных страниц (SaaS) защита `X-Frame-Options: DENY` сохранена.
>
> ✅ **Обновлено (2026-08-25, для Одноклассников):** в `frame-ancestors`
> добавлены `https://ok.ru https://*.ok.ru https://m.ok.ru` (OK грузит игру в
> iframe ok.ru / WebView m.ok.ru) и `https://vk.ru https://m.vk.ru` (новые домены
> VK). Применено sed-правкой в `security.js` (бэкап `security.js.bak-ok`), образ
> `deploy-neobrain-web` пересобран, контейнер `neobrain-web` пересоздан.
> Проверено curl — заголовок отдаётся с `ok.ru`/`*.ok.ru`/`m.ok.ru`.

## Сборка VK-версии

```bash
npm run build:vk
# → build/vk/  (index.html, manifest.json, sw.js, privacy-policy.html,
#   css/styles.css, icons/, js/ — 18 модулей: achievements, chest, cloud-sync,
#   combo, daily-login, daily, daily-puzzle, depths-map, game, levels, main,
#   missions, platform-sdk, platform, progress, rewards, shop, sound; build.json)
```

Особенности сборки (`scripts/build-vk.js`):
- `index.html` копируется **без тега `<script async src="/sdk.js">`** — он нужен
  только Яндекс Играм; на VK такой путь привёл бы к ошибке `Failed to load resource`.
- Исключены серверные/нативные модули (`board.js`, `config.js`, `utils.js`, `ui.js`,
  `native-entry.js`, `native-plugins.js`) и все `*.test.js`.
- `build.json` содержит `{ platform: 'vk', version: '1.0.0', jsModules: 18 }`.

## Деплой на 5mb2.ru (уже выполнен)

Статика лежит в `/static/games/ocean-2048/` на том же сервере, что и плейграунд:
- `index.html`, `privacy-policy.html`, `manifest.json`, `sw.js`
- `css/styles.css`, `js/` (15 модулей), `icons/`

URL для VK: **https://5mb2.ru/static/games/ocean-2048/index.html**

> Для обновления после правок: пересобрать `npm run build:vk`, скопировать
> содержимое `build/vk/` в каталог статики сервера (SCP/rsync/sftp по
> существующей схеме деплоя).

## Пошаговая инструкция публикации в VK

> Панель разработчика VK: https://dev.vk.com (войти по аккаунту VK; для создания
> мини-аппа нужен подтверждённый аккаунт разработчика).

### Шаг 1. Создать приложение
1. https://dev.vk.com → **«Создать приложение»** → тип **«VK Mini App»**.
2. Указать название: **Океан 2048**.
3. Получить **App ID** (понадобится для параметров запуска; `detectHost()` уже
   понимает `vk_app_id`).

### Шаг 2. Указать URL игры (размещение)
1. В настройках приложения открыть раздел **«Размещение»**.
2. В поле **«Адрес приложения»** указать:
   ```
   https://5mb2.ru/static/games/ocean-2048/index.html
   ```
3. Сохранить.

### Шаг 3. Проверить запуск в iframe
1. Открыть мини-апп по прямой ссылке (кнопка «Открыть» в панели).
2. Убедиться, что игра грузится **внутри VK** (без белого экрана — это
   свидетельствует о блокировке `X-Frame-Options`).
3. Проверить консоль (F12): нет ошибок загрузки ресурсов, `VKWebAppInit`
   отработал, облако отвечает; кнопка «Таблица» открывает системную
   `VKWebAppShowLeaderBoardBox` без ошибок в консоли.

### Шаг 4. Сделать игру доступной
1. В настройках приложения выставить статус **«Включено»** — после этого игра
   доступна **всем в VK по прямой ссылке** (без модерации каталога).
2. Пока статус не включён — мини-апп видят только администраторы приложения.

### Шаг 5. Подать заявку на модерацию в каталог
1. Раздел **«Каталог»** / «Публикация» в панели приложения.
2. Заполнить карточку (см. «Шпаргалка для карточки» ниже).
3. Приложить **«Описание основных сценариев»** (сценарии использования).
4. Отправить на модерацию. Одобренная игра попадает в каталог VK и доступна
   через поиск/подборки.

## Требования модерации каталога VK (чек-лист)

Официальный источник: https://dev.vk.com/ru/mini-apps/catalog/moderation.
Модерация проверяет: правила VK, работа без ошибок, отключение звука, канал
поддержки, русский язык, качественная графика, режим обучения, уникальное
название, запрос/отказ от личных данных, отсутствие похожей игры в каталоге.

| Требование | Как выполнено у нас |
|-----------|---------------------|
| Соблюдение правил VK | ✅ контент нейтральный, 3+, без насилия/азарта; покупки — только за голоса VK (игровая валюта) |
| Игра работает без ошибок и крашей | ✅ тесты, песочница, деплой на 5mb2.ru |
| Возможность **отключить звук** | ✅ кнопка звука в игре (сохраняется в настройках) |
| Канал поддержки | ✅ e-mail поддержки в карточке + `privacy-policy.html` |
| Русский язык | ✅ весь интерфейс на русском |
| Качественная графика | ✅ иконки 32–1024 px, скриншот, тема «Океан 2048» |
| **Режим обучения / «Как играть»** | ✅ экран правил/обучения в игре + описание в карточке |
| Уникальное название | ✅ «Океан 2048» — уникальное |
| Запрос/отказ от личных данных | ✅ игра не запрашивает персональные данные; политика конфиденциальности на месте |
| Не похожа на другую игру в каталоге | ✅ уникальная тема «Океан 2048» |

## Шпаргалка для карточки каталога (копируй-вставляй)

**Название игры**
```
Океан 2048
```

**Короткое описание (одной строкой)**
```
2048 в подводном мире: прилив и отлив, акула, турнир и дуэль с друзьями.
```

**Полное описание**
```
🌊 Океан 2048 — живая головоломка 2048 в подводном мире, где каждый ход — решение. Соединяй одинаковые плитки-кораллы, переживи прилив и отлив, обгони акулу-охотника и поднимайся по рангам — от Ракушки до Хозяина Моря!

🧠 ГЛУБИНА ЯДРА — ход = решение:
• 🌊 Прилив и отлив: вода периодически удваивает или делит плитки — планируй слияния под фазу роста
• 🦈 Акула-охотник: хищник преследует крупные плитки на глубинах «Акула» и «Хозяин Моря»
• ⚡ Плитки-способности: бомба 💣 убирает ряд, медуза 🪼 замораживает прилив, краб 🦀 сливает плитки
• 🎲 Случайные события: Шторм 🌪️, Медузий дождь 🪼 и Пузырь 🫧 меняют ход партии
• 🐬 Дельфин-навигатор: подскажет оптимальный ход и предупредит о тупике

🏆 СОРЕВНОВАНИЕ С ДРУЗЬЯМИ:
• 🏆 Ежедневный турнир глубин — 50 ходов на максимум очков на общей доске дня
• ⚔️ Дуэль дня — вызови друга на бой и сравни результат
• 🧩 Ежедневная головоломка — одна доска для всех, побей рекорд дня
• 👥 Приглашай друзей и получай жемчужины, бросай вызов «Побей мой рекорд»
• 🗺️ Карта глубин с сюжетными миссиями и наградами
• ⏱️ Режим «Челлендж» и еженедельные челленджи

🎨 АТМОСФЕРА:
• 7 рангов-глубин: Ракушка, Краб, Медуза, Черепаха, Осьминог, Акула и Хозяин Моря
• Поля 4×4, 5×5 и 6×6 — сложность растёт с каждым уровнем
• Живой океан: процедурный фон с рыбами, планктоном и лучами света
• Скины плиток, темы оформления и осеннее событие 🍂
• Оригинальный саундтрек (музыка и звук отключаются в настройках)
• Ежедневные задания и ежедневный бонус с наградами
• Облачные сохранения: прогресс синхронизируется между устройствами
• Таблица рекордов — соревнуйся с другими игроками
• Покупки жемчужин за голоса VK — по желанию, игровая валюта

Правила простые: сдвигай плитки, сливай одинаковые числа и получай очки. За жемчужины покупай бусты и скины. Исследуй глубины, набирай рекорды и стань настоящим Хозяином Моря!
```

**«Как играть» (режим обучения)**
```
Соединяй плитки с одинаковыми числами, сдвигая их стрелками или свайпами влево, вправо, вверх и вниз. Две одинаковые плитки сливаются в одну, а число удваивается. За каждый ход набирай очки, а за серии слияний получай бонусы.

Цель игры — пройти 7 рангов-глубин: от Ракушки до Хозяина Моря. На каждом уровне цель растёт, а поле становится больше: 4×4, 5×5 и 6×6.

Используй подсказку или отмену хода, покупай в магазине бусты «Перемешать», «Бомба» и «Двойные очки», открывай скины плиток и темы оформления. Выполняй ежедневные задания и забирай ежедневный бонус. Прогресс автоматически сохраняется в облако. Набери максимум очков и стань Хозяином Моря!
```

**Описание основных сценариев (для модерации)**
```
1. Пользователь открывает игру по ссылке из VK. Загружается заставка, затем — главное меню.
2. Нажатие «Играть» запускает новую партию: свайпами/стрелками пользователь сдвигает плитки и соединяет одинаковые числа.
3. По мере набора очков открываются новые ранги (от Ракушки до Хозяина Моря) и поля большего размера; включаются прилив и отлив, акула-охотник и случайные события.
4. В магазине за жемчужины покупаются бусты и скины; ежедневный бонус и задания дают дополнительные жемчужины.
5. В разделе «Жемчужины» пользователь покупает набор за голоса VK: открывается системное окно VKWebAppShowOrderBox, после подтверждения зачисляются жемчужины (тестовый режим до публикации в каталоге).
6. В разделе соревнований доступны ежедневный турнир, дуэль с другом и ежедневная головоломка.
7. Результаты записываются в таблицу рекордов VK; прогресс сохраняется в облако VK.
```

**Настройки карточки**

| Поле | Значение |
|------|----------|
| Адрес приложения (URL) | `https://prod-app54731343-2ec00e9a1ff3.pages-ac.vk-apps.ru/index.html` (хостинг VK) |
| Категория | Головоломки (Puzzle) |
| Язык | Русский |
| Возрастной рейтинг | 3+ |
| Политика конфиденциальности | `https://5mb2.ru/static/games/ocean-2048/privacy-policy.html` |
| Канал поддержки | **(проверить)** свой e-mail |
| Звук | есть кнопка отключения в игре ✅ |

## Временный хостинг на серверах VK (решение)

> **Решение (2026-08-21):** до готовности своей экосистемы (24/7-сервер) игру временно
> размещаем на **бесплатном хостинге VK** через `vk-miniapps-deploy`, а после переноса
> своей экосистемы — переключим URL в кабинете на свой сервер. Так игра не зависит от
> простоя нашего VPS и работает 24/7 с первого дня.

VK даёт бесплатный статический хостинг для мини-аппов через утилиту
**`vk-miniapps-deploy`**:
```bash
npm run build:vk                                   # → build/vk/ (без /sdk.js)
npx @vkontakte/vk-miniapps-deploy                  # загружает build/vk/
# выдаёт HTTPS-URL вида https://prod-app<ID>-<hash>.pages-ac.vk-apps.ru/...
```
Этот URL указываем в разделе «Размещение» — файлы хранятся на серверах VK,
свой сервер для игры не нужен. Обновление = пересборка + повторный деплой.

> ✅ **Деплой выполнен (2026-08-22):** приложение 54731343 задеплоено на хостинг VK.
> Прод-URL (все три платформы):
> - vk.ru / iOS & Android / m.vk.ru:
>   **https://prod-app54731343-2ec00e9a1ff3.pages-ac.vk-apps.ru/index.html**
> (ответ HTTP 200). Обновление игры = пересборка `build/vk` + повторный деплой той
> же командой; URL остаётся прежним.

**Перенос на свою экосистему (позже):**
1. Задеплоить `build/vk/` на свой 24/7-сервер по HTTPS (как уже сделано на 5mb2.ru).
2. Проверить, что сервер не шлёт `X-Frame-Options`/`CSP frame-ancestors`, блокирующие VK.
3. В кабинете VK (раздел «Размещение») заменить URL на свой.
4. Убедиться, что облачные сохранения (`ocean2048_save`) продолжают работать
   (они на стороне VK и от смены URL не зависят); таблица — системная
   `VKWebAppShowLeaderBoardBox`, показ не зависит от URL.

**Параллельно (не мешает VK-хостингу):** игра уже развёрнута на 5mb2.ru
(https://5mb2.ru/games/game/ocean-2048) — это наш «запасной» URL и плейграунд
до переноса экосистемы.

## Ассеты для кабинета VK (иконки, сниппет, скриншоты, Lottie)

> Генерация: `npm run make:vk` → `store/vk/`. Скрипт [`scripts/make-vk-assets.mjs`](../scripts/make-vk-assets.mjs)
> рендерит иконки из `icons/icon.svg`, сниппет — из иконки в фирменном стиле,
> скриншоты 600×1200 — центральный кроп реальных игровых скриншотов
> `store/shots/iphone-*.png` (1179×2556), Lottie — встроенная анимация 96×96.
> **Эти файлы загружаются вручную в кабинете VK** (раздел «Изображения»), в хостинг не деплоятся.

| Требование VK | Размер | Файл `store/vk/` | Готов |
|---|---|---|---|
| Универсальная иконка (каталог, лента, сообщения, рекомендации) | 576×576 PNG | `icon-universal-576.png` | ✅ 51,6 КБ |
| Иконка для каталога и сниппетов (дополнительная) | 278×278 PNG | `icon-catalog-278.png` | ✅ 19,9 КБ |
| Маленькая иконка (экран запуска, сообщения, уведомления) | 150×150 PNG | `icon-small-150.png` | ✅ 9,9 КБ |
| Фавикон (десктоп, вкладка браузера) | 32×32 PNG ≤50 КБ | `favicon-32.png` | ✅ 1,5 КБ |
| Большой сниппет | 1120×630 PNG | `snippet-1120x630.png` | ✅ 258 КБ |
| Скриншоты (экран запуска в десктопе) | 600×1200 PNG | `screenshots/screenshot-600x1200-{1-home,2-moves,3-shop,4-skins}.png` | ✅ 4 шт. |
| Анимированная иконка (экран загрузки) | 96×96 Lottie JSON ≤24 КБ | `loading-animation.json` | ✅ 1,2 КБ (2 слоя, валидный JSON) |

> Уже были: фавикон 32×32 — [`icons/icon-32.png`](../icons/icon-32.png); баннеры 1280×720 — [`store/media/banners/`](media/banners/).

## Тест в песочнице / на устройстве (перед модерацией)

- [ ] Игра открывается в iframe VK (без белого экрана, без ошибки про X-Frame-Options).
- [ ] `VKWebAppInit` отработал: в консоли нет `VKWebAppInit` ошибок, лоадер доходит до 100%.
- [ ] Свайпы/стрелки работают, плитки двигаются, партия проходит.
- [ ] Кнопка звука: выключение/включение работает и запоминается.
- [ ] «Как играть» (режим обучения) открывается из меню.
- [ ] Облачное сохранение: прогресс переживает перезагрузку (ключ `ocean2048_save`).
- [ ] Кнопка «Таблица» открывает системную `VKWebAppShowLeaderBoardBox` (без ошибок в консоли).
- [ ] Реклама (если разрешена VK): interstitial между партиями, rewarded — за награду.
- [ ] Кнопка «Поделиться» открывает VKWebAppShare.
- [ ] Никаких консольных ошибок и сетевых 404.
- [ ] На мобильном устройстве через приложение VK интерфейс не «плывёт».

## Чеклист готовности

- [x] VK-сборка `scripts/build-vk.js` + npm-скрипт `build:vk` (без тега `/sdk.js`)
- [x] Сборка `build/vk/` актуальна (18 модулей js, build.json `platform: 'vk'`)
- [x] VK SDK-адаптер: `VKWebAppInit`, облако `ocean2048_save`, реклама, шаринг
- [x] VK SDK-адаптер: лидерборд через `VKWebAppShowLeaderBoardBox` (кнопка «Таблица»
      в игре; запись через серверный `secure.addAppEvent` — **после каталога**, Фаза 4)
- [x] Деплой статики на HTTPS: https://5mb2.ru/static/games/ocean-2048/index.html
- [x] Политика конфиденциальности на HTTPS: `privacy-policy.html`
- [x] Автоопределение хоста VK (`detectHost()`) + загрузка VK Bridge
- [x] VK Mini App создан: **App ID 54731343** (https://vk.com/app54731343)
- [x] Конфиг `vk-hosting-config.json` готов для `vk-miniapps-deploy` (App ID 54731343)
- [x] Проверить, что сервер 5mb2.ru не шлёт `X-Frame-Options`/`CSP frame-ancestors`, блокирующие VK; `frame-ancestors` разрешает `vk.com`/`*.vk.com`/`*.vk.me`/`ok.ru`/`*.ok.ru`/`m.ok.ru` (обновлено 2026-08-25)
- [x] Выполнить деплой на хостинг VK (`npx @vkontakte/vk-miniapps-deploy` — авторизация + код подтверждения из сообщения VK)
- [x] Получить URL хостинга VK: **https://prod-app54731343-2ec00e9a1ff3.pages-ac.vk-apps.ru/index.html** (HTTP 200)
- [ ] Указать URL хостинга VK в разделе «Размещение» приложения 54731343 (если ещё не указан)
- [ ] Статус приложения «Включено» (доступ по прямой ссылке в VK)
- [ ] Проверить запуск в iframe VK (без белого экрана, `VKWebAppInit` без ошибок)
- [ ] **Повторная подача после модерации (2026-09-08):** сменить тип приложения на «Игра», активировать товары «Платежи» и проверить тестовые платежи — см. раздел «⚠️ Ответ модерации VK» в начале файла
- [ ] Заполнить карточку каталога + «Описание основных сценариев» и отправить на модерацию
- [x] Сгенерировать все ассеты VK (`npm run make:vk` → `store/vk/`): иконки 576/278/150/32, сниппет 1120×630, скриншоты 600×1200, Lottie JSON 96×96 (`loading-animation.json`)
- [ ] Загрузить иконки/сниппет/скриншоты/Lottie в кабинете VK (раздел «Изображения»)
- [ ] **IAP (код готов):** создать товары «Платежи» в кабинете VK — `donate_small` (10 голосов), `donate_medium` (25), `donate_large` (60), `donate_mega` (130); id должны совпадать с `DONATE_PACKS` в [`js/chest.js`](../js/chest.js) — пошагово в разделе «Ответ модерации VK» №3
- [ ] **IAP:** включить монетизацию приложения в кабинете dev.vk.com (платежи + реклама interstitial/rewarded)
- [ ] **IAP:** проверить в песочнице — каталог `VKWebAppGetOrderItems` и окно покупки `VKWebAppShowOrderBox` открываются без ошибок; после подтверждения зачисляются жемчужины
- [ ] **Одноклассники:** привязать «OK ID» (Настройки → «Другие площадки») и подать на модерацию OK — см. [`OK_GAMES.md`](OK_GAMES.md)

## Особенности / риски

- **VK без архивов**: обновление игры = пересборка `build/vk/` + обновление файлов
  на сервере по тому же URL (URL в кабинете менять не нужно).
- **X-Frame-Options** — главная причина «белого экрана» в VK. Если iframe не
  открывается — проверить заголовки сервера (см. команду выше).
- Игра открывается в iframe VK: полноэкранная кнопка не работает — код безопасно
  игнорирует отказ (`requestFullscreen().catch(() => {})`).
- Облако VK (`ocean2048_save`) не пересекается с облаком Яндекса (`ocean2048`) —
  прогресс на каждой платформе независим.
- До модерации каталога игра доступна **по прямой ссылке** при статусе «Включено» —
  этого достаточно, чтобы «люди играли»; каталог нужен для поиска внутри VK.

## Оформление сообщества VK (единый хаб «5MB2 Digital»)

> Одно сообщество — вся экосистема: игра «Океан 2048» + студия 5MB2 + SaaS NeoBrain.

- Генерация ассетов: `npm run make:community` → [`scripts/make-community-assets.mjs`](../scripts/make-community-assets.mjs)
  → `store/community/` (обложка 1590×400, аватары, промо-посты 1200×630, квадраты
  1080×1080, истории 1080×1920, рекламные креативы VK 1200×627/1080×1080/1080×1920).
  Стиль «сток-гибрид»: фото-подложка (CSS-слои или `store/community/stock/underwater.jpg`) + типографика.
- Reels (VK Клипы, 9:16): `node scripts/make-reels.mjs [сек]` → серия
  `store/media/reels/reel-{game,depths,tips,meme,news}.mp4` (+ `-preview.gif`).
- Полный гайд оформления (паспорт сообщества, готовые посты, реклама, шаги вручную):
  [`COMMUNITY_VK.md`](./COMMUNITY_VK.md).
- Сообщества в настройках мини-аппа: раздел «Официальное сообщество» → выбрать хаб
  (сообщения от игры будут идти от него).
